...
const validationSchema = Yup.object().shape({
  ...
  preferableActivity: Yup.string().required('Please select a preferable activity'),
});
...
export default function MyAccount() {
  ...
  useEffect(() => {
    setFieldValue('fullName', user?.given_name || 'johndoe');
    setFieldValue('target', user?.target || 'build_muscle');
    setFieldValue('preferableActivity', user?.preferable_activity || 'yoga');
  }, [setFieldValue, user]);
                ...
                <Grid container spacing={3}>
                  <Grid item xs={9} display='flex' alignItems='center' mb={2}>
                    <Avatar sx={{ width: 64, height: 64, mr: 2 }}>
                      {getNameInitials(user?.given_name) || 'JD'}
                    </Avatar>
                    <Box>
                      <Typography variant='h6'>
                        {user?.given_name || 'Johnson Doe'}
                      </Typography>
